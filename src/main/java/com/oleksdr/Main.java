...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("%s, number of your games is: %d%nStart the game?%n",
                playerName, numberOfGames);

        if (inputService.getAnswer().equalsIgnoreCase("n")) return;

        startGame(numberOfGames, playerName);
    }
    private static void startGame(int numberOfGames, String playerName){
        ...
        while (player.getNumberOfGames() < numberOfGames) {
            ...
            getGameResult(gameService, deckService, player);
        }
    ...
    private static void getGameResult(
        ...
        System.out.printf("Player %s: %s%nTotal score: %s%n", player.getName(), player.getPlayerHand(), playerScore);
        System.out.printf("Computer: %s%nTotal score: %s%n", computerHand, computerScore);
